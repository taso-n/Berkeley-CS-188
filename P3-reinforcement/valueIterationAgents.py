...
class ValueIterationAgent(ValueEstimationAgent):
        ...
        for iteration in range(self.iterations):
            for state in self.mdp.getStates():
                ...
                for action in self.mdp.getPossibleActions(state):
                    maxNum = max(maxNum, self.computeQValueFromValues(state, action))
                values[state] = maxNum
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for transition in self.mdp.getTransitionStatesAndProbs(state, action):
            ...
            res += transition[1] * (reward + self.discount * self.values[transition[0]])
    ...
    def computeActionFromValues(self, state):
        ...
        res = None
        maxNum = float("-inf")
        if len(self.mdp.getPossibleActions(state)) == 0: return res
        for action in self.mdp.getPossibleActions(state):
...
class AsynchronousValueIterationAgent(ValueIterationAgent):
    ...
    def runValueIteration(self):
        ...
        firstValues = self.values.copy()
        while curLength < maxLen:
            for state in states:
                if curLength >= maxLen: return
                values = firstValues.copy()
                for action in self.mdp.getPossibleActions(state):
