...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        resDist = float("inf")
        resAction = None
        minDistancedGhost = None

        for distance in livingGhostPositionDistributions:
            pos = distance.argMax()
            curDist = self.distancer.getDistance(pacmanPosition, pos)
            minDistance = min(curDist, minDistance)
            minDistancedGhost = pos


        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            curDist = self.distancer.getDistance(successorPosition, minDistancedGhost)
            if (curDist < resDist):
                resDist = curDist
                resAction = action
